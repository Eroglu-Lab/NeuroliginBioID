...
##############
  ...
  fc <- fc %>% mutate(pvalvalue= if_else(pval < 0.05, "p<0.05", "p>0.05"),
               pvalvalue = factor(pvalvalue, levels = c("p>0.05", "p<0.05")))
  
  fc$pval_rnd <- formatC(fc$pval, digits=2, format="e")
...
observeEvent(input$Gene, {
  output$FC <- renderPlot({ 
    ...
    ggplot(tab_fc(), aes(x=BioID, y=FC, fill=cell)) + 
      ...
      geom_text(aes(label=pval_rnd, color=pvalvalue,
                    fontface=if_else(pval < 0.05, "bold", "plain")), vjust=-0.9, size=6, 
      ...
      scale_fill_manual(values = c("palegreen4","hotpink4")) + scale_color_manual(values=c('black', 'red'), drop=FALSE) +
      guides(color='none')
